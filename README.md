...
The app pins published `arcane-os@0.5.11` and uses its public browser-speech, AI, DBOPFS, event, and `arcane-os/speech-playback` contracts. JuJu owns story narration, voice choice, presentation, and passage pauses. Its current `SpeechPlayback` path sends complete passages through `AI.fetchTTS`, synthesizing one passage at a time and preparing the next during playback. The configured AI streaming cadence (earliest punctuation or four whole words) applies to `AI.streamTTS`, not this passage-based playback path. JuJu's controls subscribe to the playback instance's public `SPEECH_PLAYBACK_STATE_EVENT`; destroying playback releases that subscription. The SDK owns provider registration, loading, cancellation, Worker lifecycle, WAV normalization, runtime asset loading, playback, and the single `globalThis.arcaneEvents` authority. The legacy app-local `speech-worker.js`, direct CDN import, AudioContext scheduler, and browser-speech fallback have been retired.

SDK 0.5.11 defaults to a four-request Kokoro pool with automatic WebGPU-first selection and WASM fallback. JuJu's current `SpeechPlayback` queue does not yet submit concurrent synthesis requests or use the SDK's contiguous streaming-audio scheduler. The required correction is to submit the complete current page to the SDK's punctuation-segmented generation queue immediately, preserve selected voice and paragraph pauses, and play results in order. The existing public `streamTTS` queue needs voice, pause, and playback-completion options before JuJu can use it without losing those controls. The SDK owner is adding that public seam; it is not in the installed 0.5.11 package. Do not treat a package upgrade, one-lookahead generation, or app-source preparation as completion of parallel narration.
