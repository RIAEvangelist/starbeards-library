...
import waitForComponent from './WaitForComponent.js?arcaneVersion=0.13.1';
...
async function sendWithWindowMail(target,...args){
    if(!is.function(target?.mail?.send)){
        await import('./Mail.js?arcaneVersion=0.13.1');
    }
...
async function ensureHTMLImport(target){
    ...
    try{
        await import('./HTMLImport.js?arcaneVersion=0.13.1');
    }catch(error){
        if(!registry.get('html-import')){
            throw error;
        }
