...
import waitForComponent from './WaitForComponent.js?arcaneVersion=0.25.0';
...
async function sendWithWindowMail(target,...args){
    if(!is.function(target?.mail?.send)){
        await import('./Mail.js?arcaneVersion=0.25.0');
    }
...
async function ensureHTMLImport(target){
    ...
    try{
        await import('./HTMLImport.js?arcaneVersion=0.25.0');
    }catch(error){
        if(!registry.get('html-import')){
            throw error;
        }
